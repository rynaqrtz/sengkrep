...
Before debugging anything, run `sengkrep doctor`. It reports the Node version, which optional drivers are installed, and whether the directories a scraper writes to are usable, so a missing dependency shows up as a `WARN` line instead of a confusing error later. Details in [Command line](#command-line).
...
The same scheduler is reachable from a terminal with `sengkrep jobs` and `sengkrep run`, with the job store on disk so a cadence survives a restart. See [Command line](#command-line).
...
sengkrep discover https://example.com
```

`fetch` prints the response body. `discover` prints the sitemap and `robots.txt` URLs it found, one per line.
...
sengkrep scrape https://books.toscrape.com \
  ...
  --sink '{"type":"postgres","table":"books","key":"sku"}'
```
...
sengkrep run jobs.js --watch          # keep the scheduler running until Ctrl+C
```
...
- whether each optional driver is installed: `playwright`, `pg`, `mysql2`, `@clickhouse/client`, `@aws-sdk/client-s3`
- DNS resolution for `--host`, and a DevTools endpoint when `--cdp` is given
...
`keepAlive` sockets, and the metrics server when `observability.enabled` is set, keep the event loop alive on purpose. Call `scraper.close()` when the job is done. Scripts generated by `capture.toScript()` already end with it. Use `await scraper.flush()` before closing if you also want pending background cache refreshes to finish.

### Which features work on this machine?

Run `sengkrep doctor`. It answers the questions that otherwise turn into a confusing error halfway through a run: is the Node version new enough, is `node:sqlite` available, can this build decompress zstd, are the temp and working directories writable, is the driver for the sink you chose installed, and does a DevTools endpoint answer when you pass `--cdp`. A `WARN` is an optional feature that is off; a `FAIL` exits `1`. `sengkrep doctor --json` gives the same report to a script.
