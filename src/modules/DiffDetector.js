...
class DiffDetector {
  ...
  check(url, data) {
    ...
    if (!stored) {
      const firstRunResult = { url, firstRun: true, changes: [], hasCritical: false, previousTs: null };
      this._history.push(firstRunResult);
  ...
  getAllChanges(url) {
    if (url) return this._history.filter(r => r.url === url);
    return [...this._history];
  }

  getChangedOnly(url) {
    return this.getAllChanges(url).filter(r => r.changes.length > 0);
  }
